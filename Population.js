...
class Population {
    ...
    constructor(size) {
        ...
        this.bFinished = false;                      //used to switch from playing to generating the next generation of ships
    ...
    setBestPlayer() {
        ...
        for (let i = 0; i < this.lPlayers.length; i++) {
            if (this.lPlayers[i].fFitness > max) {
                max = this.lPlayers[i].fFitness;                     //Sets max to current highest value
        ...
        if (this.lPlayers[index].nScore > this.nBestScore) {         //Sets the best score to the fittest ship's score if it is higher
            this.nBestScore = this.lPlayers[index].nScore;       
            this.bestPlayer = this.lPlayers[index].cloneForReplay(); //Clones the ship for later use.
        }
    }
    ...
    //----------------------------------------------------------------
    naturalSelection() {
        let newPlayers = [this.lPlayers.length];        //Create a new list of the same population size
        ...
        for (let i = 1; i < this.lPlayers.length; i++) {
            ...
            newPlayers[i].mutation();     //Once the ship has been generated, run it through the mutation tool to give it a possible genetic mutation.
        }

        this.lPlayers = newPlayers;  //Clone the newly created generation to replace the current one
    ...
    selectPlayer() {
        ...
        return this.lPlayers[0];                     //If somehow no ship clears the sum threshold, return the first ship (for error catching)
    }
